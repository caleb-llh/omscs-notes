...
* **Threads:** To achieve high concurrency without the massive overhead of process switching, OSes use threads. Threads are independent execution contexts (each with its own stack) that live within the *same* process address space. 
...
* **I/O Management:** The OS translates the myriad of hardware devices into a unified abstraction via the **Virtual File System (VFS)**. You will explore how the OS aggressively masks the agonizing slowness of physical disks using buffer caches, I/O scheduling, and prefetching.
