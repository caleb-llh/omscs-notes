...
> **Mental Model:** Think of RPC as an illusion. It tricks your program into thinking a function call is happening locally, while the RPC runtime does the heavy lifting of packaging the request, sending it over a network, and waiting for the result.
...
> **Tradeoff:** While RPC hides the complexity of network communication (which is great for productivity), it also masks network failures, latency, and serialization costs behind a seemingly innocent function call. What looks like a fast `add(i, j)` could unexpectedly block for seconds or fail due to network partitions.
...
> **Confusion:** Early RPC models heavily favored synchronous (blocking) execution to perfectly mimic local procedure calls. However, in modern distributed systems, this can lead to performance bottlenecks. Many modern frameworks (like gRPC) now heavily utilize asynchronous RPCs to avoid blocking the client thread.
...
> **Mental Model:** A stub is essentially a "stand-in" or a proxy. The client stub pretends to be the actual server function to the client, while the server stub pretends to be the client to the actual server function. They bridge the gap between the local execution environment and the network.
...
> **Confusion:** The terms "marshaling" and "serialization" are often used interchangeably, but historically, marshaling implies gathering data from different memory locations and formatting it for transmission (often including type and structural metadata), while serialization strictly refers to converting a data structure into a byte stream. In RPC contexts, marshaling includes serialization.
...
> **Mental Model:** The IDL is the binding legal contract between the client and the server. As long as both parties adhere to the signatures defined in the IDL, they can be implemented in completely different languages (e.g., a Python client talking to a C++ server) and still perfectly understand each other.
...
> **Tradeoff:** A centralized registry (like a single DNS or service discovery node) is easy to query and manage but becomes a single point of failure. A decentralized registry (like a portmapper on every host) distributes the failure risk but requires the client to already know which host to contact, pushing the discovery problem one layer up.
...
> **Confusion:** You cannot send a raw memory address over the network because the client and server have completely isolated address spaces. Address `0x1000` on the client means nothing to the server. RPC systems "fake" pointer passing by eagerly dereferencing the pointer, sending the actual data, and having the server allocate new memory for it.
...
> **Mental Model:** Think of this like the "Two Generals' Problem". RPC failures are ambiguous. If a client times out, it cannot know if the request was lost on the way to the server, if the server crashed while processing it, or if the reply was lost on the way back. This ambiguity makes exactly-once semantics nearly impossible without complex state management.
...
> **Tradeoff:** SunRPC uses XDR (language-agnostic), which allows cross-language interoperability but requires developers to learn a new syntax. Conversely, frameworks like Java RMI use language-specific interfaces directly, which is frictionless for developers in that ecosystem but locks them into it.
