...
> **Mental Model:** The OS as a Government or Manager. The OS doesn't inherently do "useful" work for the end user (like playing a video or editing text). Instead, it provides a safe, fair, and orderly environment, allocating resources (memory, CPU time) and enforcing rules so that applications can run efficiently without stepping on each other's toes.
...
> **Mental Model:** The Illusionist. The OS gives each program the illusion that it has exclusive use of a massive, contiguous block of memory and full control of the CPU. It hides the messy reality of shared hardware, fragmented memory, and time-slicing.
...
> **Tradeoff:** Isolation vs. Performance. High isolation (e.g., completely separate address spaces, virtual machines) prevents buggy or malicious applications from crashing the system, but it adds overhead (costly context switching, memory duplication). Lower isolation is faster but riskier.
...
> **Common Confusion:** Mechanism vs. Policy. A *mechanism* is the "how" (e.g., the low-level ability to evict a memory page to disk). A *policy* is the "what" or "when" (e.g., choosing *which* page to evict using LRU). Hardcoding policy into a mechanism leads to inflexible systems.
...
> **Common Confusion:** User-mode vs. Kernel-mode Transitions. Applications *never* simply "switch" to kernel mode on their own. They must trigger a hardware *trap* or make a *system call*, which safely pauses the application and hands control to the OS via a secure, predefined hardware pathway.
...
> **Tradeoff:** System Calls vs. Function Calls. System calls are significantly slower than regular function calls due to the user/kernel mode switch, saving hardware state, and cache pollution (evicting data from CPU caches). This is why bulk operations (e.g., buffered I/O) are preferred over single-byte system calls.
...
> **Tradeoff:** Monolithic vs. Microkernel. Monolithic kernels (like Linux) offer high performance because everything runs in the same address space (no IPC overhead). Microkernels offer high reliability and security (a crashed device driver doesn't crash the whole OS), but they suffer from severe IPC overhead due to constant context switching.
