...
> **Mental Model:** Think of the OS as a restaurant waiter. You (the application) don't need to go to the kitchen, find the ingredients, and cook the food (the hardware). You just ask the waiter for a meal, and they handle the complex details of getting it for you.
...
> **Intuition:** Policy is the *what* and the *why* (e.g., "we need to evict the least recently used item to save space"), while mechanism is the *how* (e.g., "we will use a doubly-linked list and move accessed items to the head").
...
> **Common Confusion:** It's easy to think a user-kernel context switch is just about changing a bit and jumping to new code. However, the real performance killer is often the *cache invalidation* (TLB flush, cache pollution) that happens because the kernel needs to access different memory regions.
...
> **Mental Model:** A system call is like a controlled "safe room" door. You can't just walk into the kernel's memory space. Instead, you put your request in a specific box (registers), ring the bell (trap instruction), and wait. The kernel opens its side, checks your request, does the work safely, puts the result back, and opens your side of the door.
...
> **Intuition:** Monolithic kernels put all the eggs in one basket (fast, but if one thing crashes, everything might crash). Microkernels put everything in separate baskets and use messages to talk (safer and modular, but communication overhead is high).
...
> **Mental Model:** A process is like a house, and threads are the people living inside. Changing processes means moving to a completely different house (expensive, need to pack everything). Changing threads just means letting a different person in the same house take a turn at the kitchen counter (they already share the fridge and tools).
...
> **Common Confusion:** A process does *not* go directly from the **waiting** (blocked) state back to the **running** state. It must first go to the **ready** state and wait for the scheduler to pick it again.
...
> **Intuition:** Pipelining is like an assembly line in a factory. It's incredibly efficient because each worker does one specialized task repeatedly (high cache locality). However, if one worker is slower than the rest, the entire line backs up (bottleneck).
...
> **Mental Model:** A mutex is the key to the room where the shared data lives. A condition variable is a waiting room outside. If you enter the room and find the data isn't ready for you, you go to the waiting room and sleep. When someone else changes the data, they poke you in the waiting room to try again.
...
> **Gotcha:** Even if you unlock the mutex before signaling, spurious wake-ups can still occur at the OS level due to how thread scheduling and signals are implemented. This is why the `while(condition_is_false)` loop is mandatory when waiting on condition variables!
...
> **Mental Model:** Imagine a narrow bridge where cars can only go one way. If two cars enter from opposite sides (acquiring locks in different orders), they get stuck in the middle (deadlock). If all cars agree to only ever enter from the North side (strict lock ordering), traffic always flows and deadlocks are impossible.
...
> **Common Confusion:** Interrupts are hardware-to-CPU notifications (e.g., "keyboard key pressed"). Signals are OS-to-process notifications (e.g., "you accessed bad memory" or "please terminate"). Interrupts are handled by the kernel; signals are handled by the user process.
