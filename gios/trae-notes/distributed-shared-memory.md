...
> **Mental Model:** Think of DFS as a client-server architecture where the server is the source of truth, whereas DSM is more like a cooperative peer-to-peer network where memory is collectively pooled and managed.
...
> **Key Concept:** In a peer distributed application, every node acts as both a client (requesting data) and a server (providing data). There is no centralized bottleneck, but coordination becomes significantly harder.
...
> **Analogy:** Imagine a group of people co-authoring a Google Doc, but without a central server. They must constantly pass messages to each other about what they are typing so everyone sees the same document. This is what DSM consistency protocols do for memory.
...
> **Tradeoff:** Hardware DSM offers significantly lower latency and transparent operation, but at a steep financial cost and lack of flexibility. Software DSM is cheaper and highly flexible, but introduces higher overhead due to OS/runtime intervention.
...
> **Mental Model:** False sharing is like two people needing to use different tools that happen to be stored in the same heavy toolbox. Even though they don't need each other's tools, they keep wrestling control of the entire toolbox back and forth, wasting energy and time.
...
> **Tradeoff:** Migration saves memory and simplifies consistency (since there's only one copy), but causes high latency if multiple nodes need access. Replication allows fast concurrent reads but consumes more memory and requires complex, expensive consistency protocols for writes.
...
> **Tradeoff:** Proactive (push) consistency ensures data is always up-to-date but can waste network bandwidth if the updated data is never read. Reactive (pull) consistency saves bandwidth by only fetching data when needed, but adds latency to read operations.
...
> **Mental Model:** Strict consistency is the "speed of light" problem. Since information cannot travel faster than light, and networks have inherent latency, making a write *instantaneously* visible to a node halfway across the world is physically impossible.
...
> **Key Concept:** Sequential consistency doesn't care about real time. It only cares that there is *some* logical timeline of events that every node agrees upon. As long as everyone sees the same illusion, the system is sequentially consistent.
...
> **Analogy:** Think of weak consistency like working on a local branch in Git. You can make many commits locally (writes) without anyone else seeing them. It's only when you explicitly `git push` (release/sync) and others `git pull` (acquire/sync) that the changes are synchronized across the team.
