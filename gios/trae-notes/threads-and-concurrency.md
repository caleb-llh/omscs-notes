...
> **🧠 Mental Model: The House and the Inhabitants**
> Think of a process as a house (the address space) and threads as the people living in it. They share the same rooms and appliances (code, data, heap), but each person has their own current task and memory of what they're doing (CPU registers, stack).
> 
> **⚠️ Common Confusion: PCB vs. TCB**
> It's easy to confuse the PCB (Process Control Block) and TCB (Thread Control Block). A process has one PCB, but in a multithreaded environment, the PCB points to multiple TCBs—one for each thread's distinct execution context.
...
> **⚖️ Tradeoff: Multithreading vs. Multiprocessing**
> Multithreading saves memory and makes context switching faster because the address space remains the same, but it requires careful synchronization. Multiprocessing provides strong isolation (if one process crashes, others survive) at the cost of higher memory overhead and expensive Inter-Process Communication (IPC).
...
> Concurrency is about *dealing* with multiple things at once (hiding latency like disk I/O by switching contexts), while parallelism is about *doing* multiple things at once (multi-core execution). A single CPU can achieve concurrency to stay busy during wait times, even if it cannot achieve true parallelism.
...
> **⚠️ Common Confusion: Data Races vs. Race Conditions**
> A data race occurs when two threads concurrently access the same memory location and at least one is a write. A race condition is a broader flaw in the timing or ordering of events leading to incorrect program behavior. A program can have race conditions without data races!
>
> **🧠 Mental Model: Mutexes and Condition Variables**
> A mutex is like a restroom key. Only the thread holding the key can enter the critical section. A condition variable is like a waiting room where you sit until a receptionist (another thread) calls your name (signals) that the resource you need is ready.
...
> **⚖️ Tradeoff: Dynamic Thread Creation vs. Thread Pools**
> `fork`/`join` implies creating threads on the fly, which introduces initialization overhead. In performance-critical applications, pre-allocating a pool of threads at startup and assigning them work from a queue avoids the runtime cost of constant thread creation and destruction.
...
> **🧠 Mental Model: The Stale Reference Problem**
> Imagine two people trying to update the same shared document simultaneously without locking it. They both read the current state, make their edits locally, and hit save. The last one to save overwrites the other's work completely. Mutexes force them to take turns reading and writing.
...
> **⚠️ Common Confusion: Mutexes Protect Code, Not Data**
> Mutexes do not magically lock the data itself. They only protect the *code* in the critical section. If some other rogue thread accesses the shared data without first acquiring the mutex, the data can still be corrupted. Mutual exclusion relies on the cooperative discipline of all threads.
...
> **🧠 Mental Model: Busy Waiting vs. Pagers**
> Busy-waiting (looping to check a condition) is like a kid in the backseat repeatedly asking "Are we there yet?". It burns CPU cycles that could be used for actual work. Condition variables act like a restaurant pager system: "Go to sleep, we'll buzz you when your table is ready."
...
> **⚖️ Tradeoff: Signal vs. Broadcast**
> `signal` wakes up exactly one waiting thread, which is efficient but risky if the woken thread cannot actually proceed (it might go back to sleep, leaving the system stuck). `broadcast` wakes up all waiting threads; it's safer when multiple threads could potentially proceed, but it causes the "thundering herd" problem where many threads wake up and immediately block trying to acquire the mutex.
...
> **🧠 Mental Model: The Proxy Variable as a Sign-in Sheet**
> Instead of putting a single lock on the library door (which would block multiple readers from entering together), we put a lock on a sign-in sheet at the front desk. You only lock the sign-in sheet to update your status, then you unlock it and go into the library to read.
...
> **⚠️ Common Confusion: "I locked the mutex, why is my program still wrong?"**
> Locking the wrong mutex, locking different mutexes for the same shared state, or forgetting to lock it in just one code path completely invalidates the mutual exclusion guarantee. It's an all-or-nothing system.
...
> **🧠 Mental Model: The Stolen Table**
> A spurious wake-up is like your restaurant pager going off, but by the time you walk up to the host stand, someone else has swooped in and taken the table. You just go back to the waiting area. This is why condition variables must *always* be checked in a `while` loop, never a simple `if` statement.
...
> **🧠 Mental Model: Traffic Jams and Philosophers**
> Deadlock is a traffic jam where every car is blocking another car in a cycle, and nobody can move. In OS theory, it's often modeled as the "Dining Philosophers" problem, where everyone is holding one chopstick and refusing to let go while waiting for a second one.
>
> **⚖️ Tradeoff: Deadlock Prevention vs. Detection**
> Prevention (like strict lock ordering) is restrictive and requires careful design, but has low runtime overhead. Detection (building wait graphs) allows more flexible coding but requires expensive runtime monitoring and a way to safely abort/rollback threads.
...
> **⚖️ Tradeoff: Kernel Threads vs. User Threads**
> User-level threads are lightweight and fast to context-switch (no system calls), but a blocking I/O operation by one user thread blocks the entire kernel thread it maps to. Kernel-level threads are heavier (require syscalls to create/switch) but allow true OS-level concurrency and I/O overlapping.
...
> **🧠 Mental Model: Real-World Multithreading Patterns**
> *   **Boss/Worker:** A restaurant manager taking all orders and handing them out to a pool of cooks.
> *   **Pipeline:** An assembly line. One thread installs the engine, another paints the car. Great for cache locality!
> *   **Layered:** A corporate hierarchy. Requests pass through departments (layers), and any employee in that department can handle the subtask.
