...
> **Mental Model:** Think of I/O devices as the "limbs and senses" of a computer. The CPU and memory are the "brain," but they need I/O devices to interact with the outside world.
...
> **Confusion:** Are command and status registers just regular memory? Yes and no. They act like memory locations from the CPU's perspective (via Memory-Mapped I/O), but writing to them directly triggers the device's microcontroller logic rather than just storing arbitrary data like RAM.
...
> **Tradeoff:** Shared Bus (PCI) vs. Dedicated Links (PCIe). A shared bus is simpler and cheaper to implement but limits total bandwidth because only one device can transmit at a time. PCIe uses dedicated point-to-point lanes, increasing bandwidth and isolating traffic, but at the cost of more complex switching hardware.
...
> **Mental Model:** Device drivers are the "translators" between the OS's generic abstractions (like reading a file or sending a packet) and the device's specific, proprietary hardware registers and commands.
...
> **Confusion:** What is the exact difference between block, character, and network devices? 
> - **Block devices** have addressable, fixed-size chunks (you can ask for block 5 specifically). 
> - **Character devices** are continuous streams of bytes (like a keyboard typing), with no concept of addressable chunks. 
> - **Network devices** deal in discrete packets (chunks of varying size), which doesn't neatly fit the block or character model.
...
> **Tradeoff:** Interrupts vs. Polling. 
> - **Interrupts** free the CPU to do other work but incur context-switching overhead and cache pollution when they fire. 
> - **Polling** avoids context switches and interrupt overhead, but wastes CPU cycles if the device is not ready. 
> High-performance systems often use a hybrid approach (e.g., NAPI in Linux) where they poll under high load (when data is always ready) and use interrupts under low load.
...
> **Mental Model:** PIO (Programmed I/O) is like the CPU manually carrying buckets of water (data) one by one to a pool (the device). It's simple but occupies the CPU completely.
...
> **Mental Model:** DMA (Direct Memory Access) is like the CPU turning on a hose. The CPU sets up the transfer (turns the valve) and goes back to other work, while the DMA controller handles the actual data movement directly from memory.
...
> **Tradeoff:** DMA setup overhead vs. PIO simplicity. For very small transfers, the cost of configuring the DMA controller exceeds the time it takes the CPU to just copy the data via PIO. For large transfers, DMA is vastly superior because it frees the CPU.
...
> **Mental Model:** OS Bypass is like giving a user direct access to the kitchen instead of ordering through a waiter (the OS kernel). It's much faster, but you need strict rules (hardware-enforced isolation) so users don't interfere with each other.
...
> **Tradeoff:** Kernel mediation vs. OS Bypass. Kernel mediation provides safety, sharing, and standard interfaces, but adds latency via system calls and context switches. OS Bypass minimizes latency and CPU overhead (great for High-Performance Computing and low-latency networking) but requires specialized hardware and user-level driver support.
...
> **Confusion:** Does async I/O mean the CPU isn't doing the work? Yes, in true async I/O, the hardware (like a DMA controller or a separate storage controller) handles the data transfer, allowing the CPU to execute other threads in the meantime.
...
> **Mental Model:** VFS is the ultimate "interface" (in the OOP sense) for filesystems. It allows a Linux system to seamlessly mount an ext4 disk, a FAT32 USB drive, and a network file system (NFS) side-by-side, presenting them as a single unified directory tree to user applications.
...
> **Confusion:** What is the difference between a file descriptor, an inode, and a dentry? 
> - A **file descriptor** is a per-process handle (integer) to an open file. 
> - An **inode** is a persistent representation of the file's data and metadata on disk. 
> - A **dentry** is an in-memory cache object representing a path component (like a directory name) to speed up path traversal.
...
> **Mental Model:** An inode is like a book's table of contents, telling the OS exactly which disk blocks contain the file's data. If you want block 5 of a file, the OS looks at the 5th entry in the inode's block list.
...
> **Tradeoff:** Direct vs. Indirect pointers. Direct pointers are fast (one disk read to find the block) but severely limit file size since the inode size is fixed. Indirect pointers allow massive files by chaining blocks of pointers, but require multiple disk reads (inode -> pointer block -> data block), which increases access latency.
...
> **Tradeoff:** Caching/Buffering vs. Data Integrity. Keeping writes in memory (buffer cache) vastly improves performance by batching and reordering them. However, a sudden power loss means those writes are lost. Tools like `fsync` and journaling help balance this tradeoff by enforcing persistence when required.
...
> **Mental Model:** Journaling is like a restaurant taking your order on a notepad before actually cooking the food. If the chef (system) crashes, they can look at the notepad (journal) to see what needs to be cooked upon recovery, rather than losing the order entirely.
