...
> **Mental Model: The Checkout Line vs. The Assembly Line**
> The boss/workers model is like multiple independent checkout lanes in a grocery store—each lane handles an entire customer's order from start to finish. The pipeline model is like an assembly line—each worker does one specific, repetitive task and passes the item along. 
> 
> **Tradeoff: Latency vs. Throughput**
> Pipeline architectures often increase the latency of an individual item (because of the overhead of passing data between stages) but can dramatically improve overall throughput (by keeping all specialized stages constantly busy). Boss/workers minimizes overhead for single items but can bottleneck if tasks are uneven or require specialized resources.
> 
> **Confusion: "Faster" is Relative**
> When asked "which is faster?", the answer depends entirely on the metric. A pipeline might complete a batch of 100 items sooner (lower total execution time), while boss/workers might finish the *first* item sooner (lower average time to completion).
...
> **Mental Model: The Metric Spectrum**
> Metrics can be broadly categorized into three buckets: **Time** (latency, wait time, response time), **Rate** (throughput, bandwidth, requests per second), and **Resource** (CPU utilization, platform efficiency, performance per watt).
> 
> **Tradeoff: Utilization vs. Latency**
> Pushing a system to 100% CPU utilization often degrades latency because new tasks have to wait in a queue for resources to free up. Designing for low latency usually requires keeping utilization lower (leaving idle headroom) to handle bursts of traffic.
> 
> **Confusion: Average vs. Percentiles (Tail Latency)**
> Averages hide outliers. An average response time of 10ms might mean 99 requests take 1ms and 1 request takes 901ms. System designers often care more about the 95th or 99th percentile (p99) latency—ensuring that even the worst-case experiences remain within acceptable bounds.
...
> **Mental Model: Apartments vs. Houses**
> A multiprocess model is like a neighborhood of separate houses. Each house (process) has its own kitchen (memory), offering great isolation and security, but sharing food (data) requires walking outside (IPC). A multithreaded model is like roommates in an apartment—they share the kitchen, making sharing easy but requiring coordination (synchronization) to avoid bumping into each other.
> 
> **Tradeoff: Isolation vs. Efficiency**
> Multiprocess provides fault isolation (one crashing process doesn't kill others) at the cost of higher memory footprint and expensive context switches. Multithreaded provides memory efficiency and cheap context switches at the cost of vulnerability (one segfault brings down the entire process) and synchronization complexity.
...
> **Mental Model: The Juggler**
> An event-driven model is like a single juggler (the thread) keeping many balls (requests) in the air. The juggler only touches one ball at a time (handling an event), but by rapidly switching attention to whichever ball is falling, they maintain the illusion of processing all of them simultaneously. 
> 
> **Tradeoff: Simplicity of State vs. Blocking Peril**
> Event-driven programming avoids the need for locks because only one handler runs at a time (shared state is naturally safe). However, the tradeoff is the *blocking peril*: if a handler makes a blocking I/O call (like reading from disk), the juggler stops completely, and all the balls fall to the ground.
> 
> **Confusion: Concurrency vs. Parallelism**
> Event-driven single-threaded systems are *concurrent* (managing multiple tasks simultaneously by interleaving them) but not *parallel* (executing multiple tasks at the exact same physical instant). 
...
> **Mental Model: The Sous-Chef Delegation (AMPED)**
> If the head chef (event dispatcher) has to run to the store (blocking I/O), the entire kitchen halts. The AMPED (Asymmetric Multi-Process Event-Driven) model solves this by hiring sous-chefs (helpers). The head chef hands the shopping list to a sous-chef. While the sous-chef blocks (goes to the store), the head chef continues cooking other dishes.
> 
> **Tradeoff: Asynchronous I/O vs. Portability**
> Native Asynchronous I/O (AIO) is the ideal solution (the OS handles the wait), but historically it lacked portability and consistent implementation across different UNIX variants. The AMPED model faked AIO using blocking helpers, trading a slight increase in memory footprint (for the helper processes) for universal portability and robustness.
> 
> **Confusion: select/poll vs. epoll**
> `select` and `poll` require the OS to scan *every* provided file descriptor to see which one is ready, making them inefficient as connections scale (O(N)). `epoll` maintains a queue of ready descriptors inside the kernel, allowing the application to just pop ready events instantly (O(1)), solving the C10k problem.
...
> **Mental Model: The Scientific Method in Systems**
> Systems research isn't just about building something fast; it's about proving *why* it's fast. A testbed is a controlled laboratory. The workload is the stimulus, the system is the subject, and the metrics are the vital signs.
> 
> **Tradeoff: Synthetic Workloads vs. Real Traces**
> Synthetic workloads (like requesting the exact same file repeatedly) are great for isolating specific behaviors (like best-case cache performance) but rarely reflect reality. Real traces (like the Owlnet or CS traces) capture realistic access patterns but make it harder to pinpoint exactly *why* a system behaves a certain way due to chaotic variables.
> 
> **Confusion: "Apples to Apples"**
> A common mistake is varying multiple parameters simultaneously (e.g., changing both the threading model AND the cache size). If performance improves, you can't prove which change caused it. You must fix all variables except the one being tested to isolate causality.
