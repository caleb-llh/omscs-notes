...
> **Mental Model:** Think of the three-tier architecture like a restaurant. The presentation tier is the menu and dining room, the business logic tier is the kitchen preparing the food, and the database tier is the pantry where ingredients are stored.
...
> **Terminology:** **Scale out** (horizontal scaling) means adding more nodes to the system, while **scale up** (vertical scaling) means adding more resources (CPU, RAM) to an existing node.
...
> **Tradeoff:** Homogenous architectures simplify load balancing and frontend routing at the cost of poor cache utilization, as consecutive requests for the same data might be sent to completely different nodes.
...
> **Tradeoff:** Heterogenous architectures offer excellent cache locality and specialized performance, but significantly increase the complexity of load balancing and system management during failures or traffic spikes.
...
> **Mental Model:** Traditional infrastructure is like buying a car—you pay for it whether you drive it or not, plus you handle maintenance. Cloud computing is like taking a taxi or rideshare—you only pay for the exact amount of transportation you use.
...
> **Real-World Example:** E-commerce sites on Black Friday perfectly illustrate the need for elastic scaling. Without elasticity, they would have to over-provision hardware that sits idle for the remaining 364 days of the year just to handle the single day of peak traffic.
...
> **Key Concept:** **Statistical Multiplexing** relies on the law of large numbers. Because different customers have different peak usage times, their resource demands average out, allowing cloud providers to oversubscribe physical hardware without impacting performance.
...
> **Mental Model:** Think of Cloud Service Models as making a pizza. On-premises is cooking from scratch at home. IaaS is buying the ingredients (dough, cheese, sauce) but baking it yourself. PaaS is ordering a frozen pizza and just baking it. SaaS is going to a pizzeria where everything is done for you.
...
> **Tradeoff:** In multi-tenant cloud environments, providers must balance resource utilization with strict isolation. Maximizing hardware utilization can sometimes lead to "noisy neighbor" problems if isolation mechanisms (like hypervisors or containers) aren't perfectly tuned.
...
> **Key Concept:** **Virtualization** is the fundamental building block of cloud computing. By abstracting hardware into virtual machines or containers, cloud providers can quickly allocate, migrate, and destroy resources on demand without hardware changes.
...
> **Mental Model:** The cloud democratizes big data. Previously, only massive corporations could afford the hardware to analyze petabytes of data. Now, a startup or student can rent a supercomputer-equivalent cluster for a few hours to run the same analysis.
