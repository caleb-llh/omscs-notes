...
> **Mental Model:** Think of virtualization as an apartment building. The physical hardware is the entire building, and each virtual machine is an individual apartment. Tenants (operating systems) feel like they have their own independent home, but they all share the same foundation, plumbing, and electricity.
...
> **Key Concept:** Fidelity, performance, and safety are the three pillars of virtualization. A VMM must emulate hardware accurately (fidelity), run code fast (performance), and keep VMs separate (safety).
...
> **Tradeoff:** While consolidation saves physical space, power, and hardware costs, it introduces a single point of failure. If the underlying physical machine crashes, all VMs running on it go down as well.
...
> **Mental Model:** Bare-metal virtualization is like a dedicated manager directly interacting with workers (hardware), eliminating middle management (a host OS) for maximum efficiency.
...
> **Tradeoff:** Hosted virtualization is easier to install and leverages existing host OS drivers, but it suffers from higher overhead since operations must pass through the host OS before reaching the hardware.
...
> **Mental Model:** Think of rings as security clearances. The hypervisor holds top-secret clearance (Ring 0), forcing the guest OS to take a slightly lower clearance (Ring 1) to prevent it from directly altering physical hardware state.
...
> **Key Concept:** Trap-and-emulate is the cornerstone of processor virtualization. The guest OS attempts a privileged action, the CPU intercepts it (trap), and the hypervisor safely mimics the result (emulate).
...
> **Tradeoff:** Binary translation allows running unmodified guest OSes (Full Virtualization), but adds computational overhead because instructions must be analyzed and translated on the fly.
...
> **Mental Model:** Paravirtualization is like a self-aware simulated reality. The guest OS knows it is in a VM and cooperates with the hypervisor (via hypercalls) instead of blindly trying to execute privileged instructions.
...
> **Tradeoff:** Shadow page tables avoid the massive overhead of translating memory addresses twice in software, but they consume more memory and require the hypervisor to meticulously synchronize state on every context switch.
...
> **Tradeoff:** The passthrough model provides excellent performance by letting the VM directly access the device, but it sacrifices device sharing and makes VM migration extremely difficult.
...
> **Mental Model:** The split device driver model is like a restaurant kitchen. The front-end driver (waiter) takes the standardized order from the guest VM and hands it to the back-end driver (chef in the host/service VM) who interacts with the physical ingredients (hardware).
