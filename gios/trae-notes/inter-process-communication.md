...
> **Mental Model:** Think of a process as a walled garden. IPC mechanisms are the gates, bridges, and messenger pigeons that allow different gardens to exchange resources or coordinate activities without breaking down the walls.
...
> **Tradeoff:** Message-based IPC shifts the burden of synchronization and memory management to the OS, simplifying application code. However, this comes at the cost of multiple context switches and data copies (user-space to kernel-space and back), reducing overall throughput.
...
> **Mental Model:** A pipe is like a one-way water hose connecting two processes. You pour bytes in one end and they come out the other in the exact same order. There are no boundaries between distinct "messages"—it's just a continuous stream.
...
> **Tradeoff:** Unlike pipes, message queues preserve message boundaries and allow for priority-based delivery. The tradeoff is increased complexity in managing message types and formatting compared to a simple byte stream.
...
> **Mental Model:** Sockets abstract communication as "endpoints" that can connect across the globe or on the same machine. Local sockets (e.g., Unix domain sockets) bypass the network stack but maintain the same API as internet sockets, allowing developers to scale local IPC to distributed IPC seamlessly.
...
> **Mental Model:** Shared memory is like two people looking at the same whiteboard. Either can write or read instantly without a middleman, but if they try to write over each other at the same time, it results in a garbled mess.
...
> **Tradeoff:** Memory mapping (Shared Memory) has a high initial setup cost (modifying page tables, TLB shootdowns) but near-zero cost per data transfer. Message passing (Copying) has low setup cost but incurs high per-transfer costs due to memory copying and context switches.
...
> **Common Confusion:** SysV shared memory segments outlive the process that creates them. If a process crashes without explicitly destroying the segment, the memory remains allocated system-wide until rebooted or manually removed (e.g., via `ipcrm`). This is a common source of memory leaks in IPC applications.
...
> **Mental Model:** `ftok` is essentially a deterministic hash function that allows disconnected processes to independently arrive at the same unique "key" by hashing a known file path and a project ID.
...
> **Common Confusion:** POSIX shared memory doesn't use "real" files on the disk, even though it uses file descriptors and `shm_open`. It uses `tmpfs` (a RAM-based filesystem), meaning data operations are strictly memory-speed and vanish on reboot, despite the file-like API.
...
> **Tradeoff:** Shared memory provides the highest possible IPC performance but shifts the entire burden of concurrency control (mutexes, semaphores, condition variables) to the developer. Bugs here often lead to subtle race conditions that are incredibly difficult to debug.
...
> **Common Confusion:** It is not enough to simply set `PTHREAD_PROCESS_SHARED`. The mutex or condition variable object *itself* must be allocated inside the shared memory region. If you initialize a shared mutex in local process memory, other processes cannot access it!
...
> **Mental Model:** Using message queues for synchronization is like using a token-passing system. If you need a lock, you consume the token (receive a message). When you're done, you return the token (send a message). The queue's inherent blocking behavior acts as the wait mechanism.
...
> **Tradeoff:** A single large shared memory segment requires you to write a custom memory allocator for that region, which is highly complex. A pool of smaller, fixed-size segments is easier to manage but can lead to internal fragmentation and requires chunking for large messages.
