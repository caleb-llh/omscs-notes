...
> **Mental Model:** Think of a program as a recipe in a cookbook (static) and a process as the actual act of cooking the meal (active). You can cook the same recipe multiple times simultaneously, each with its own ingredients and progress (state).
...
> **Confusion:** It's easy to confuse the heap and the stack since both are dynamically created state. Remember that the stack is managed automatically by the compiler/OS for function calls (LIFO), while the heap is managed explicitly by the programmer (e.g., `malloc`/`free`) and can become fragmented ("holes filled with garbage").
...
> **Mental Model:** The page table is like a giant lookup directory that translates a process's fake (virtual) addresses into real (physical) addresses in RAM. This illusion allows every process to believe it owns the entire memory space, preventing processes from interfering with each other's memory.
...
> **Tradeoff:** Swapping to disk allows the OS to support more processes and larger address spaces than physical memory permits, but disk access is orders of magnitude slower than RAM. The tradeoff is between system capacity and execution speed.
...
> **Mental Model:** The PCB (Process Control Block) is the "save state" or "snapshot" of a process. Just like saving a video game allows you to resume exactly where you left off, the OS saves the PCB so it can pause and later resume a process seamlessly.
...
> **Tradeoff:** Frequent context switching makes the system feel more responsive to the user (fairness and interactivity), but it incurs significant overhead (direct CPU cycles and indirect cache misses). The OS must balance responsiveness with overall throughput.
...
> **Confusion:** Why does `fork` create an exact copy of the parent just for `exec` to immediately throw it away and load a new program? This two-step process historically allowed the child to inherit the parent's environment (like file descriptors) before `exec` runs. Modern OSes often use optimizations like "copy-on-write" to avoid actually duplicating memory during a `fork`.
...
> **Mental Model:** The CPU scheduler is like a traffic cop at a busy intersection, deciding which process gets to go next and for how long. The timeslice is the length of the green light.
...
> **Tradeoff:** Message Passing vs. Shared Memory IPC. Message passing is safer and easier to synchronize because the OS handles the communication channel, but it is slower due to data copying overhead. Shared memory is incredibly fast because it bypasses the OS, but it requires the programmer to manually implement complex synchronization to avoid race conditions.
