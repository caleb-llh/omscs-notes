...
> **Mental Model:** Think of the CPU as a single cashier at a busy grocery store, and the scheduler as the store manager deciding which customer (thread) gets served next based on store policies (scheduling algorithms).
...
> **Common Confusion:** Context switching is not free! Many people assume switching between threads happens instantly, but saving and restoring registers, memory maps, and CPU state takes real time and consumes CPU cycles that could otherwise be used for actual work.
...
> **Intuition:** MLFQ learns the behavior of a thread over time. If a thread hogs the CPU, it gets demoted to a lower priority with a longer timeslice. If it does a lot of I/O, it stays at a high priority with a short timeslice to keep the system responsive.
...
> **Mental Model:** A page table is like a restaurant's seating chart. The virtual address is the reservation name, and the physical address is the actual table they are seated at. The MMU is the host who checks the chart and walks you to your table.
...
> **Pro Tip:** Copy-on-Write (COW) is a lazy allocation strategy. The OS avoids doing expensive memory copying upfront, delaying the work until it is absolutely necessary (when a write actually occurs).
...
> **Common Confusion:** "Swapping" and "Demand Paging" are often used interchangeably, but historically swapping meant moving entire processes to disk, while demand paging moves individual pages. Modern OSes use demand paging.
...
> **Intuition:** The TLB is just a hardware cache specifically for the page table. Since memory access is slow, and walking a page table requires multiple memory accesses, the TLB memorizes recent translations to bypass the page table entirely.
...
> **Mental Model:** A hierarchical page table is like a multi-level directory structure. Instead of having one massive flat folder with millions of files (which takes up huge space), you have nested folders where you only create the inner folders when you actually need to store a file there.
...
> **Mental Model:** Shared memory is like two different houses (processes) having back doors that open into the exact same shared courtyard (physical memory). They might have different door numbers (virtual addresses), but they step into the same physical space.
...
> **Mental Model:** RPC makes calling a function on a remote server look and feel exactly like calling a local function in your own program. The runtime hides all the messy network serialization and socket management.
...
> **Intuition:** The sharing granularity represents a tradeoff between overhead (network traffic) and precision. Too large (e.g., page level) leads to false sharing where independent variables mistakenly trigger coherence updates; too small (e.g., variable level) creates excessive tracking metadata and network messages for small changes.
...
> **Common Confusion:** Consistency models are not about *how* data is stored, but rather the *rules* or *contracts* about when updates made by one node become visible to other nodes in the system.
...
> **Intuition:** Cloud scaling relies on the law of large numbers. A provider can pool resources across thousands of users because not everyone will hit peak usage at the same time. The downside is that with thousands of components, statistically speaking, something is always failing.
