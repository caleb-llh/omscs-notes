...
> **Mental Model:** Think of a mutex like waiting in your car with the engine off (relinquishing the CPU), whereas a spinlock is like keeping the engine running and constantly revving it while waiting for a parking spot to open (burning CPU cycles).
...
> **Common Confusion:** It's easy to confuse a binary semaphore with a mutex. While they function similarly in ensuring mutual exclusion, a mutex is typically "owned" by the thread that locks it (and must be unlocked by the same thread), whereas a semaphore is just a counter and can be signaled by any thread.
...
> **Tradeoff:** Reader/Writer locks increase concurrency by allowing multiple readers simultaneously. However, this comes at the cost of complexity and potential starvation. For example, if a continuous stream of readers arrives, a waiting writer might starve unless the implementation explicitly prioritizes writers.
...
> **Mental Model:** A monitor is like a bouncer at an exclusive club (the shared resource). You don't need to manually check IDs or manage the line (lock/unlock); the bouncer handles all the synchronization logic automatically as you enter and leave.
...
> **Mental Model:** Atomic instructions are the indivisible atoms of synchronization. They bundle multiple operations (like reading, modifying, and writing) into a single, unbreakable hardware step that cannot be interrupted or interleaved by other threads.
...
> **Tradeoff (Write-Invalidate vs. Write-Update):** Write-Invalidate uses less bandwidth since it only sends addresses to invalidate, but forces a full memory read on the next access. Write-Update saves the cost of a main memory read by keeping caches warm, but consumes more interconnect bandwidth sending the actual data on every write.
...
> **Tradeoff:** To guarantee safety, atomic instructions bypass the cache and go directly to main memory, generating coherence traffic. The tradeoff is strict correctness at the steep cost of performance (latency and bus contention), making atomics inherently expensive.
...
> **Common Confusion:** Why test twice? The first test (`while (lock == busy)`) is a normal read that spins on the local cached copy without triggering expensive memory bus traffic. The second test (`test_and_set`) is the actual atomic operation used only when the lock appears free.
...
> **Tradeoff:** Static delays are simple but can cause unnecessary waiting under low contention. Dynamic delays adapt to contention levels (often using exponential backoff) but introduce overhead in calculating the delay and randomness that might still lead to occasional collisions.
...
> **Mental Model:** A Queueing Lock is like taking a ticket at the deli counter. Instead of everyone shouting at once when the clerk is free (contention), each person waits quietly for their specific number to be called, eliminating a thundering herd.
