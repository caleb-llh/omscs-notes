...
> **Mental Model:** Think of the filesystem API as a universal translator. Whether the data is on a spinning hard drive, an SSD, or a USB stick, the OS translates generic commands like `open()` and `read()` into device-specific instructions.
...
> **Tradeoff:** Replication vs. Partitioning. Replication gives you high availability and fault tolerance but costs more storage and complicates writes. Partitioning gives you infinite scalability (just add more machines) but reduces fault tolerance if a single partition goes down.
...
> **Real-world Analogy:** A peer-to-peer DFS is like a potluck dinner. Everyone brings a dish (storage/files) and everyone eats (acts as a client), sharing the overall load without a central caterer.
...
> **Design Note:** System design frequently oscillates between "move the data to the computation" (upload/download) and "move the computation to the data" (true remote access). The optimal solution is usually somewhere in the middle.
...
> **Key Concept:** Cache Coherence. The moment you introduce a cache, you introduce the problem of keeping that cache in sync with the source of truth (the server). This is the fundamental challenge of the compromise model.
...
> **Tradeoff:** Stateless vs. Stateful. Stateless servers are highly resilient to crashes (just restart them, no data lost) but require fatter network requests. Stateful servers enable advanced features like caching and locking, but require complex crash recovery mechanisms to restore their state.
...
> **Mental Model:** In a DFS, network latency breaks the illusion of instantaneous updates. You must accept that different nodes might briefly disagree on what a file looks like.
...
> **Key Concept:** Session Semantics shifts the synchronization burden to the boundaries of file access (open/close). What happens while the file is open is isolated from the rest of the network.
...
> **Performance Note:** Hotspots occur when partitioning strategies fail to distribute load evenly. If everyone is requesting the same "viral" file stored on one machine, that single machine becomes a bottleneck despite the overall system being partitioned.
...
> **Mental Model:** VFS (Virtual File System) acts as a traffic cop. It inspects the file path and directs local paths to the local disk driver, and network paths to the NFS client, all while the user application remains completely unaware of the difference.
...
> **Tradeoff:** Lease-based locking vs. Permanent locking. Leases prevent a crashed client from permanently holding a lock and freezing the system, but require active clients to periodically spend network resources to renew their leases.
...
> **Performance Note:** When files are opened and closed rapidly, session semantics (which syncs on every open/close) causes a massive spike in network traffic, defeating the purpose of the cache.
...
> **Key Concept:** Data-driven design. The Sprite DFS team didn't guess the 30-second threshold; they derived it directly from their empirical observation that many files are deleted shortly after creation.
...
> **Tradeoff:** Sprite handles concurrent writes by completely disabling caching. This sacrifices performance in the concurrent case to ensure absolute consistency and simplify the cache coherence protocol.
