...
## Scheduling Overview
> **Mental Model:** Think of the CPU as a single restaurant kitchen and the tasks as orders. The scheduler is the expeditor who decides which order the chefs work on next.
> 
> **Tradeoff:** Scheduling algorithms fundamentally trade off between *fairness* (everyone gets a turn) and *performance* (getting things done as quickly as possible with minimal overhead).
...
> **Common Confusion:** "Wait time" vs "Completion time". Wait time is only the time spent sitting in the ready queue, while completion time includes wait time *plus* actual execution time.
> 
> **Tradeoff:** FCFS is perfectly fair in terms of arrival, but suffers from the "convoy effect" where a single massive job blocks all short jobs behind it, destroying average wait time.
...
> **Mental Model:** Preemption is like the kitchen expeditor pausing a chef in the middle of making a 12-hour brisket because an urgent 2-minute salad order just came in.
> 
> **Common Confusion:** True SJF is impossible in general-purpose OSes because it requires predicting the future (exact execution time). It's primarily used as a theoretical benchmark for optimal wait times.
...
> **Common Confusion:** Starvation isn't a deadlock. The task *could* run, but the scheduler actively chooses not to run it due to an endless stream of higher-priority tasks. Priority aging solves this by making waiting tasks "angrier" (higher priority) over time.
...
## Priority Inversion
> **Mental Model:** Priority Inversion is like a VIP (T1) waiting for a normal guest (T3) to leave a private room (mutex), but the normal guest keeps getting interrupted by a manager (T2). Thus, the VIP is effectively delayed by the manager.
> 
> **Tradeoff:** Priority Inheritance (boosting) solves priority inversion but adds complexity to the mutex and scheduler implementation, requiring the OS to track lock ownership and dynamically adjust priorities.
...
> **Tradeoff:** Timeslice length determines the balance between *responsiveness* and *throughput*. Shorter timeslices feel more responsive to the user but waste more CPU cycles on context-switching overhead.
...
> **Mental Model:** I/O bound tasks just want a quick moment on the CPU to fire off their next I/O request (like sending an email). CPU bound tasks want to sit down and crunch numbers uninterrupted for hours.
...
> **Mental Model:** The Multi-Level Feedback Queue (MLFQ) is like a sorting hat. It assumes every task is a quick, interactive task at first. If the task overstays its welcome (uses its full timeslice), it gets demoted to a "batch processing" queue with longer timeslices but lower priority.
...
## Linux O(1) Scheduler
> **Common Confusion:** "O(1)" means the time it takes to pick the next task is constant, regardless of whether there are 10 tasks or 10,000. It achieves this using bit manipulation and a fixed array of priority lists, rather than searching a single long list.
...
## Linux CFS Scheduler
> **Mental Model:** CFS aims for an egalitarian utopia. Instead of static priorities, it tracks `vruntime` (virtual runtime). The task that has had the *least* CPU time (smallest `vruntime`) is always picked next. "Nice" values simply make virtual time pass faster or slower for a task.
> 
> **Tradeoff:** Red-Black Trees give O(log N) insertion, which is technically slower than O(1)'s constant time, but CFS provides much better fairness and avoids the awkward "expired queue" latency spikes of O(1).
...
> **Mental Model:** Cache affinity is like a worker preferring to stay at the desk where all their reference books are already open. Moving to a different desk (CPU) means having to fetch all the books from the library (Main Memory) again.
...
## Hyperthreading
> **Common Confusion:** Hyperthreading (SMT) does *not* mean you have two physical CPUs. It means you have one CPU pipeline with two "front desks" (hardware contexts). If one thread stalls on memory, the other can immediately use the pipeline without an OS-level context switch.
...
> **Tradeoff:** Using Hardware Performance Counters (HPCs) allows the OS to make incredibly informed scheduling decisions (e.g., pairing a memory-bound thread with a CPU-bound thread), but parsing this hardware data adds overhead to the scheduler itself.
