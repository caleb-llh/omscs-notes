...
playlists = [
    "PLAwxTw4SYaPmqpjgrmf4-DGlaeV0om4iP",
    "PLAwxTw4SYaPkNw98-MFodLzKgi6bYGjZs",
    "PLAwxTw4SYaPnhRXZ6wuHnnclMLfg_yjHs",
    "PLAwxTw4SYaPn79fsplIuZG34KwbkYSedj",
    "PLAwxTw4SYaPkr-vo9gKBTid_BWpWEfuXe",
    "PLAwxTw4SYaPndXEsI4kAa6BDSTRbkCKJN"
]
...
for i, pl_id in enumerate(playlists):
    ...
    for vtt in vtt_files:
        ...
        # Remove the 001_ prefix and .en.vtt
        title = re.sub(r'^\d+_', '', filename).replace('.en.vtt', '').replace('.vtt', '').strip()
        # Clean up some common youtube suffixes if present
        title = title.replace(" - Georgia Tech - HPCA", "").replace(" - Georgia Tech", "").strip()
        
        # Start new module on Introduction or if module is getting too large
...
print("All HPCA transcripts fetched and grouped!")
