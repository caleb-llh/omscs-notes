...
> **Key Concept:** Routing is about determining the path, while forwarding is about moving the packet within a single router.
...
> **Background Context:** Intradomain routing assumes all routers are under a single administrative domain, which simplifies the routing protocols compared to interdomain routing.
...
> **Design Philosophy:** Intradomain routing is an exercise in distributed consensus. Do you give everyone a complete map so they can calculate their own route (Linkstate), or do you just rely on rumors from your neighbors (Distance Vector)? Both solve the same problem but optimize for different constraints.
...
> **Intuition:** Because every node has a complete map of the network, each can independently calculate the shortest path to all other nodes without relying on neighbors' calculations.
...
> **Key Concept:** Dijkstra's algorithm builds a shortest-path tree iteratively, ensuring the lowest cost path is found for each node.
...
> **Tradeoff:** While Linkstate provides a complete and accurate view of the network, the $O(n^2)$ complexity and broadcasting overhead make it less scalable for massive networks compared to distance-vector approaches.
...
> **Purpose:** Distance Vector routing is designed to allow nodes to find shortest paths purely through local conversations, eliminating the need for a global network map.
...
> **Tradeoff:** Distance Vector algorithms converge very quickly when costs decrease ("good news travels fast"), but suffer from routing loops and slow convergence when costs increase ("bad news travels slowly").

## **Poison Reverse**

> **Mental Model:** Poison reverse is like telling a specific neighbor a path is blocked to prevent them from routing back through you.
...
> **Background Context:** Implementing RIP as an application-level process (using UDP) rather than a strict network-layer protocol simplifies its design and allows it to leverage standard socket APIs.
...
> **Key Concept:** Hot potato routing minimizes internal transit costs by handing off traffic to an external network at the closest possible exit point.
...
> **Tradeoff:** While hot potato routing saves internal network resources, it may result in a longer overall end-to-end path for the packet, shifting the transit burden to the neighboring AS.
