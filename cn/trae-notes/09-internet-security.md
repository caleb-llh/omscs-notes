...
> **Design Philosophy:** The original Internet was built by researchers who trusted each other, meaning security was an afterthought rather than a foundational primitive. Today's network security is essentially a massive, ongoing effort to patch trust onto a system inherently designed to be open.

> **Key Concept:** Secure communication fundamentally relies on Confidentiality, Integrity, Authentication, and Availability to protect against attackers.
...
> **Mental Model:** Think of **DNS abuse** like a criminal constantly moving their hideout and leaving fake forwarding addresses to evade the police.
...
> **Intuition:** RRDNS is like a receptionist at a busy office who directs each new caller to the next available agent in a circular order.
...
> **Purpose:** CDNs reduce latency and increase availability by storing copies of content geographically closer to the end users.
...
> **Tradeoff:** While low TTLs allow legitimate CDNs to adapt quickly to network changes, they also enable malicious actors to rapidly cycle through compromised proxy IPs.
...
> **Key Concept:** Rogue networks are primarily distinguished from legitimate ones by the longevity of their malicious behavior, sometimes lasting for months.
...
> **Background Context:** While data plane monitoring observes actual traffic, control plane monitoring looks at routing behaviors like BGP announcements and withdrawals.
...
> **Intuition:** Misconfigurations, such as open DNS resolvers or untrusted certificates, act as symptoms of poor overall security management.
...
> **Mental Model:** BGP hijacking is like changing the road signs on a highway so that all traffic destined for a legitimate town is redirected to a malicious one.

> **Purpose:** BGP hijacking can be used for various malicious activities, including intercepting traffic, blackholing destinations, or impersonating services.
...
> **Background Context:** When a shorter or equally short path is announced for a prefix, routers that believe the new path will direct traffic to the hijacker.
...
> **Tradeoff:** When configuring a detection system like ARTEMIS, network operators must balance the desire for high accuracy against the risk of false positives.
...
> **Key Concept:** DDoS attacks leverage compromised hosts (slaves) to overwhelm a victim, often using spoofed IP addresses to amplify the attack and hide the attacker's identity.
...
> **Mental Model:** A **reflection and amplification attack** is akin to mailing thousands of catalog requests with someone else's return address, causing their mailbox to overflow with heavy catalogs.
...
> **Tradeoff:** Scrubbing services provide fine-grained filtering of attack traffic but come with significant financial costs and potential latency overhead.
...
`{ “type 1”: "172.217.19.195/32” “type 2": "130.89.161.0/24" "type 3": [6], "type 5": [80, 443], "action": { "type ": "traffic-rate", “value ": "0" } }`
...
> **Intuition:** BGP blackholing stops the attack traffic but also blocks legitimate users, effectively completing the attacker's goal of denying service.
