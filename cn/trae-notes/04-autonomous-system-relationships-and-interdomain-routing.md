...
> **Purpose:** IXPs exist to keep local traffic local, reducing latency and avoiding transit costs that would otherwise be paid to higher-tier ISPs.
...
> **Background Context:** BGP is designed for policy and reachability across different administrative domains, whereas IGPs are strictly for performance and shortest-path routing within a single domain.
...
> **Key Concept:** Exporting routes is inherently tied to financial incentives; an AS generally only exports routes that generate revenue or are required by agreement.
...
When an AS receives multiple route advertisements towards the same destination, from multiple ASes, then it needs to rank the routes before selecting which one to import. The routes that are preferred first are the customer routes, then the peer routes and finally the provider routes. 

> **Intuition:** The ranking of Customer > Peer > Provider is purely driven by economics. Customers pay you, peers are free, and you pay providers.

The reasoning behind this ranking is that an AS...
...
> **Tradeoff:** BGP prioritizes local autonomy and policy flexibility over finding the absolute shortest path, meaning traffic often takes suboptimal routes from a strict latency perspective.
...
> **Mental Model:** Think of eBGP as the language between different companies, and iBGP as the internal memos within the same company spreading that external information.
...
> **Purpose:** iBGP ensures that all routers within an AS have a consistent view of the external routes learned via eBGP, without modifying the next-hop attribute.
...
> **Background Context:** While LocalPref controls outbound traffic by dictating exit points, MED attempts to control inbound traffic by suggesting entry points to neighboring ASes.
...
> **Tradeoff:** Flap damping improves overall network stability by isolating unstable routes, but it can temporarily sacrifice reachability for valid prefixes that just experienced a legitimate sequence of changes.
...
> **Mental Model:** IXPs are like massive network hubs or airports where different ASes can directly exchange traffic without paying a third-party provider.
...
Generally, the manner in which two ASes exchange traffic through the switching fabric was utilizing a two-way BGP session, called a **bilateral** BGP session. Since there has been an increasing number of ASes peering at an IXP, we have another challenge to accommodate an increasing number of BGP sessions. Obviously this option does not scale with a large number of participants. > **Key Concept:** A Route Server simplifies peering at IXPs by replacing a full mesh of BGP sessions with a single multi-lateral peering session.

To mitigate this some IXPs operate a route server, which helps to make peering more manageable. In summary, a Route Server (RS):
...
Steps:
  ...
  3. The route server applies the peer-specific export filter to check if AS X allows AS Z to receive p1, and if true it adds that route to the AS Z-specific RIB. Now, RS advertises p1 to AS Z with AS X as the next hop.
...
> **Key Concept:** Ensuring BGP configuration correctness is critical because misconfigurations can lead to widespread internet outages or routing loops.
...
This chapter covers the fundamental concepts, algorithms, and protocols related to the topic, providing a comprehensive understanding of routing mechanisms and network relationships.
