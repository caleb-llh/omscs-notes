...
> **Historical Context:** The "Galactic Network" vision was revolutionary for its time, laying the conceptual groundwork for a globally interconnected set of computers long before the technology existed.
...
> **Milestone:** The transition from connecting hardware to developing standardized protocols allowed software applications (like email) to leverage the network.
...
> **Scalability Solution:** DNS replaced a centralized hosts file with a distributed hierarchy, demonstrating how decentralized systems can solve massive scaling problems.
...
> **Mental Model:** Think of network architecture like an airline system, where each step (ticketing, baggage, gate, takeoff) represents a distinct layer with specific functionalities.

After looking at the major milestones in the history of the Internet, let's take a closer look 
into the current architectural design of the Internet.
...
> **Key Concept:** The OSI model provides a conceptual framework for understanding network interactions across seven distinct layers, whereas the Internet architecture combines the top three into a single application layer.
...
> **Key Concept:** Encapsulation is the process of adding layer-specific headers to the **payload** as it moves down the stack, while de-encapsulation strips them away as it moves up.
...
> **Key Concept:** NAT boxes violate the end-to-end principle by actively modifying IP addresses and ports in the network core, rather than leaving this to the endpoints.
...
> **Design Philosophy:** The Hourglass Model forces a single, universal protocol (IP) at the waist. This "narrow waist" guarantees interoperability—any application above it can run on any hardware below it, as long as both speak IP.
...
This chapter provided an overview of the history of the Internet and its architectural principles. We explored the OSI and Internet layered models, encapsulation, the end-to-end principle, NAT, the hourglass shape of the Internet architecture, the EvoArch model, and learning bridges.
