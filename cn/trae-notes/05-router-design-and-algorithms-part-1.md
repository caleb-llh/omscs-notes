...
> **Background Context:** The transition from software-based routing to hardware-based switching was necessary to keep up with the exponential growth of internet traffic.
...
> **Intuition:** Think of the input port as the receptionist of the router, verifying the packet's credentials and deciding exactly which hallway (switching fabric) it needs to take.

There are several functionalities that are performed by the input ports.

If we look at the figure from left to right, the first function is to physically terminate the incoming links to the **router**. Second, the data link processing unit decapsulates the packets. Finally, and most importantly, the input ports perform the lookup function. At this point, the input ports consult the **forwarding table** to ensure that each packet is forwarded to the appropriate output port through the switch fabric.
...
> **First Principles:** Routers face a brutal physical constraint: they must make decisions at the speed of light. Every nanosecond spent looking up an IP address or enforcing a rule in software is a bottleneck, forcing network engineers to bake algorithmic logic directly into silicon (ASICs).
...
> **Tradeoff:** Longest prefix matching provides more flexibility and efficiency in IP address allocation, but requires much more complex and computationally expensive lookup algorithms than exact matching.
...
> **Background Context:** Early routers were essentially just standard computers with multiple network interface cards, using the CPU and RAM to switch packets. This architecture couldn't scale to modern speeds.
...
> **Purpose:** CIDR (Classless Inter-Domain Routing) was introduced to slow down the rapid exhaustion of IPv4 addresses and to reduce the size of routing tables in the global internet.
...
Two final notes on the unibit trie:
  ...
  1. If a prefix is a substring of another prefix, the smaller string is stored in the path to the longer (more specific prefix). For example, P4 = 1 _is a substring of P2 = 111_ , and thus P4 is stored inside a node towards the path to P2.
...
For example we substitute (expand) P3 = 11001 _with 110010_ and 110011*.
