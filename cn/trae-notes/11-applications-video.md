...
> **Design Philosophy:** The Internet was built to reliably deliver text, not real-time video. Streaming protocols represent a massive engineering effort to trick an asynchronous, best-effort network into delivering synchronous, time-sensitive media without changing the underlying IP architecture.

> **Key Concept:** Multimedia applications involve audio or video and have strict timing and delay requirements compared to traditional applications.
...
> **Mental Model:** Think of video as high-bitrate but easily compressible, while audio is lower-bitrate but highly sensitive to glitches.
...
> **Key Concept:** Multimedia can be categorized into stored streaming, live streaming, and conversational (VoIP).
...
> **Mental Model:** VoIP must survive the "best effort" Internet by managing encoding, signaling, and Quality of Service (QoS).
...
> **Background Context:** The Internet was initially designed for data transfer where reliability mattered more than timing. VoIP must overlay real-time requirements on top of this asynchronous, best-effort foundation.
...
> **Key Concept:** End-to-end delay must ideally stay below 150ms; anything above 400ms makes conversational interaction nearly impossible.
...
> **Intuition:** Think of jitter as cars arriving at a toll booth at unpredictable intervals. A jitter buffer acts like a parking lot that holds early arrivals so everyone can proceed at a steady, perfectly spaced pace.
...
> **Mental Model:** In VoIP, a packet is considered "lost" if it never arrives OR if it arrives too late to be played back.
...
> **Tradeoff:** Adding redundant data (FEC) increases the ability to recover from packet loss, but it simultaneously increases the bandwidth overhead and can introduce additional playout delay.
...
> **Background Context:** The shift from downloading full files to streaming was made possible by the intersection of higher broadband speeds, smarter compression algorithms, and robust DRM for copyright protection.
...
> **Key Concept:** Video compression leverages both spatial redundancy (within a frame) and temporal redundancy (across frames) to save bandwidth.
...
> **Intuition:** Why resend the entire background if only a person's lips are moving? P-frames just send the "diffs" or changes from the previous frame, drastically reducing the required bits.
...
> **Tradeoff:** VBR (Variable Bitrate) provides higher overall video quality by using more data for complex scenes, but makes bandwidth estimation harder than CBR (Constant Bitrate).
...
> **Intuition:** Because P-frames and B-frames depend entirely on previous I-frames, a single lost packet can corrupt a whole sequence of frames. TCP's guaranteed delivery prevents this cascading corruption.
...
> **Purpose:** The playout buffer acts as a shock absorber. By pre-fetching content, it insulates the viewer's playback experience from the inevitable spikes and dips in internet throughput.
...
> **Mental Model:** DASH dynamically adjusts video quality on the fly based on current network conditions and buffer status.
...
The video player has no way of knowing that the bandwidth has reduced. Therefore it ends up requesting a 3 Mbps chunk. To download this chunk, it will take 3 _1000_ 5 / 375 seconds or 40 seconds. Meanwhile the video player buffer will deplete and the video will eventually re-buffer. If the player uses weighted average then it may even take more time to reflect the drop in the network bandwidth and the player may end up requesting higher bitrate than it should.
...
## Chapter Summary
This chapter explored the unique challenges and solutions associated with multimedia applications on the Internet. We examined the characteristics of video and audio, diving into VoIP and its critical QoS metrics: delay, jitter, and packet loss. Furthermore, we discussed video compression techniques, including spatial and temporal redundancy, and detailed how streaming works over HTTP. Finally, we analyzed dynamic bitrate adaptation (DASH) and the trade-offs between rate-based and buffer-based adaptation strategies.
