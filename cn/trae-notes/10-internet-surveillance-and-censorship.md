...
> **First Principles:** The Internet interprets censorship as damage and routes around it. However, as state actors gain control over the physical infrastructure (like ISPs and BGP routes), the battle shifts from software workarounds to a war over the underlying physical and logical topology.

> **Background Context:** The Great Firewall of China (GFW) is one of the most well-known and sophisticated examples of state-sponsored Internet censorship.

> **Key Concept:** **DNS censorship** relies on manipulating DNS responses, such as injecting fake DNS records, to block access to objectionable domains.
...
> **Mental Model:** **DNS injection** works like a corrupt directory assistance service that intentionally gives you the wrong phone number for certain people.
...
> **Intuition:** DNS injection works by racing the legitimate DNS response; if the fake response arrives first, the client accepts it and the real response is ignored.
...
> **Key Concept:** **Packet dropping** is an inexpensive censorship method but suffers from overblocking, especially when multiple websites share a single IP address.
...
> **Tradeoff:** DNS poisoning avoids the overblocking issues of IP packet dropping, but it only affects users relying on the poisoned DNS resolvers.
...
> **Mental Model:** Proxy-based content inspection is like a border checkpoint where every piece of mail is opened and read before being allowed to pass.
...
> **Purpose:** By injecting TCP RST packets, the censor forces both the client and the server to tear down their connection immediately.
...
`cam(53382) → china(http) [SYN] china(http) → cam(53382) [SYN, ACK] cam(53382) → china(http) [ACK] cam(53382) → china(http) GET / HTTP/1.0 china(http) → cam(53382) HTTP/1.1 200 OK (text/html) etc. . . china(http) → cam(53382) . . . more of the web page cam(53382) → china(http) [ACK]`
...
> **Intuition:** The firewall temporarily "remembers" a client that requested flagged content and automatically resets any subsequent requests for a set duration.
...
> **Key Concept:** Detecting global censorship is challenging due to the diversity of filtering techniques, the scale required for measurement, and the ethical risks to volunteers.
...
> **Background Context:** Distinguishing intentional censorship from benign network failures or misconfigurations requires careful analysis of multiple signals.
...
> **Purpose:** Using open DNS resolvers within the Internet infrastructure, rather than volunteer home networks, scales the measurement while minimizing ethical risks.
...
> **Mental Model:** A routing disruption via BGP is like wiping a country off the global map—routers worldwide are told the destination no longer exists.

> **Background Context:** State-controlled Internet infrastructure makes it significantly easier to enact nation-wide Internet blackouts by manipulating BGP routes.
...
> **Key Concept:** Systems like Augur use the global IP ID counter and TCP resets to remotely detect inbound and **outbound blocking** between hosts without needing access to them.
...
> **Intuition:** If the site's SYN-ACK never reaches the reflector, the reflector won't generate a RST, meaning its IP ID counter will not increment.
...
> **Tradeoff:** Because outbound blocking prevents the RST from reaching the site, the site retransmits the SYN-ACK, causing the reflector's IP ID to increment multiple times.
