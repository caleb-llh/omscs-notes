...
> **Background Context:** Early internet traffic was mostly best-effort (like email and file transfers), but modern applications like VoIP and video conferencing demand strict performance guarantees, necessitating these classification rules.
...
> **Tradeoff:** Linear search is extremely simple to implement and update, but its O(N) lookup time makes it unscalable for core routers handling millions of packets per second.
...
> **Intuition:** Passing labels is like giving a VIP wristband at the door; once the initial check is done, all subsequent checkpoints just look for the wristband instead of re-verifying the ID.
...
> **Key Concept:** Quality of Service (QoS) mechanisms like Token Bucket help shape and police traffic to ensure fair bandwidth allocation.
...
**Challenge.** The problem that we need to solve now, is which source prefixes to store at the sources tries? For example, let's consider the destination D = 00*. Both rules R4 and R5 have D as the destination prefix. So the source tries for D will need to include the source prefixes 1* and 11*. But if we restrict to 1* and 11*, then this is not sufficient. Because the prefix 0*, also matches 00*, and it is found in rules R1, R2, R3, R7. So we will need to include all the corresponding source prefixes.

> **Tradeoff:** Set-pruning tries offer fast O(W) lookup times by duplicating rules across multiple tries, leading to a massive memory explosion (O(N^2) in the worst case).
...
> **Purpose:** Backtracking sacrifices lookup speed to keep memory usage strictly linear O(N), ensuring that every rule is stored exactly once.
...
> **Intuition:** Output queuing is like having a multi-lane highway leading into a toll booth instead of a single lane; cars going to different destinations don't have to wait behind a blocked car.
...
> **Tradeoff:** Running the switching fabric N times faster completely eliminates HOL blocking, but is physically impossible or prohibitively expensive for routers with many high-speed ports.
...
A practical implementation of this approach is the Knockout scheme. It relies on breaking up packets into fixed size (cell). We suppose that in practice the same output rarely receives N cells and the expected number is k (smaller than N). Then we can have the fabric running k times as fast as an input link, instead of N. We may still have scenarios where the expected case is violated. To accommodate these scenarios, we have one or more of a primitive switching element that randomly picks the chosen output:
  ...
  * k needs to be chosen out of N possible cells, with k and N arbitrary values. We create k knockout trees to calculate the first k winners. The drawback with this approach is that is it is complex to implement.
...
The reasons to make scheduling decisions more complex than FIFO with tail drop are:
  ...
  * **Providing QoS guarantees to flows.** One way to enable fair sharing is to guarantee certain amounts of bandwidths to a flow. Another way is to guarantee the delay through a router for a flow. This is noticeably important for video flows – without a bound on delays, live video streaming will not work well.
...
Consider a flow α\alphaα. Let a packet of size `p` bits arrive as the `i`-th packet in the flow. If it arrives at an empty queue, it reaches the head of the queue at the current round `R(t)`. If not, it reaches the head after the packet in front of it finishes it. Combining both the scenarios, the round number at which the packet reaches the head is given by
...
This chapter provided an in-depth look at the fundamental concepts and mechanisms underlying the topics discussed. We explored key architectures, algorithms, and practical applications that drive modern network design.
