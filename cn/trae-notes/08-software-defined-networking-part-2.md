...
> **Paradigm Shift:** SDN moves away from distributed, device-centric control towards logically centralized, software-centric control.
...
> **Advantage:** By abstracting middlebox services as applications, SDN eliminates the need for complex physical placement and manual configuration of specialized hardware.
...
> **Key Concept:** A Network Operating System (NOS) does for the network what a traditional OS does for a computer—it abstracts the underlying hardware and provides common services for applications.
  ...
  7. **Network programming languages:** Network programmability can be achieved using low-level or high-level programming languages. Using low-level languages, it is difficult to write modular code, reuse it and it generally leads to more error-prone development. High level programming languages in SDNs provide abstractions, make development more modular, code more reusable in control plane, do away with device specific and low-level configurations, and generally allow faster development. Some examples of network programming languages in SDNs are Pyretic, Frenetic, Merlin, Nettle, Procera, FML, etc.
     ...
     * For a tutorial on Frenetic programming language: <http://frenetic-lang.github.io/tutorials/Introduction/> (Links to an external site.) Pyretic: <https://github.com/frenetic-lang/pyretic/wiki> (Links to an external site.)
...
> **Interoperability:** Open standards in the SDN infrastructure layer break vendor lock-in, allowing operators to mix and match hardware from different manufacturers.
...
> **Evolution:** While OpenFlow provided the initial standard for data plane control, P4 introduced true protocol independence, allowing developers to define exactly how packets are parsed and processed.
...
> **Use Case:** Software-Defined Internet Exchanges (SDX) apply SDN principles to inter-domain routing, solving many of BGP's long-standing limitations regarding traffic engineering and policy enforcement.
