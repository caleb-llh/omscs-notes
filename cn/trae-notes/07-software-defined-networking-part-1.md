...
> **Design Philosophy:** In computer science, every problem can be solved by adding a level of indirection. SDN applies this to networking by abstracting the distributed, hardware-bound control plane into a centralized, software-bound Network Operating System, prioritizing flexibility over rigid, decentralized autonomy.

> **Background Context:** The need for SDN emerged from the inherent complexity and rigidity of traditional network architectures.
...
> **Core Principle:** SDN is fundamentally about decoupling the brain (control plane) from the brawn (data plane) of the network.
...
> **Historical Context:** Active networks were a radical departure from the end-to-end principle, proposing that the network core should be programmable rather than dumb.
...
> **Mechanism:** OpenFlow operates on a simple "match-action" paradigm, turning complex routing logic into a set of highly optimized rule evaluations.
...
> **Advantage:** Software-based control allows network operators to use modern software engineering practices for network configuration.
...
> **Summary:** In SDN, the intelligence is centralized in the software controller, leaving the hardware switches as simple, fast packet-forwarding engines.
